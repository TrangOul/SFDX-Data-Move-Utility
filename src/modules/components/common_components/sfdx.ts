...
export class Sfdx implements IAppSfdxService, IFieldMapping {
  ...
  /**
   *  Performs SOQL query to org and returns records.
  ...
  async queryOrgAsync(soql: string, useBulkQueryApi: boolean, useQueryAll?: boolean): Promise<Array<any>> {
    ...
    async function ___makeQueryAsync(soql: string): Promise<QueryResult<object>> {
   ...
   * @memberof Sfdx
   */
  async queryOrgOrCsvAsync(soql: string,
